...
// Functions to compute moments. The formulas are generated by dev/ureal2.py.
// The function compute_mom provides an unified interface to compute_mom1,2,etc.
// Note that these are uncentered moments. In the formulas, G stands for
// gradient, H for half hessian, V for moment of elementary variables. The
// indices run over elementary variables and are summed over. Remember that
// the elementary variables are standardized, so V_ij = delta_ij. It is
// intended that different indices can not take on the same value in the sum,
// formulas with coupled indices are written separately because the iteration
// can be optimized and more importantly because moments of different order
// arise since we have the independency assumption on elementary variables.
    ...
    namespace internal {
        ...
        Real compute_m3(const HessGrad<Real> &hg) {
            ...
            for (ConstDiagIt it = hg.cdbegin(); it != dend; ++it) {
                ...
                // Update: they now are, so understand what I was doing and
                // do it.
        ...
        Real compute_c2(const HessGrad<Real> &hga, const HessGrad<Real> &hgb) {
            ...
            // Update: I don't understand anymore what I was meaning.
